...
source ./stages/image_setup.sh
source ./stages/build_kernel.sh
...
source ./tools/build_emulationstation.sh
...
source ./stages/finishing_touches.sh
source ./stages/cleanup_filesystem.sh
source ./stages/write_rootfs.sh
source ./stages/clean_mounts.sh
source ./stages/create_image.sh
) 2>&1 | tee -a build.log

echo "RG351P build completed. Final image is ready."
